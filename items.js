...
function ItemDAO(database) {
		...
		//MongoClient.connect('mongodb://localhost:27017/mongomart', function(err, db) {
			...
			this.db.collection('item').aggregate(query).toArray(function(err, docs) {
				...
				docs.forEach(function(doc) {
					categories.push(doc);
					numAll = numAll + doc.num;
				});
				
				//this.db.close();
				categories[0].num = numAll;
				callback(categories);
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         */
		 ...
		 //MongoClient.connect('mongodb://localhost:27017/mongomart', function(err, db) {
			...
			//console.log("Successfully connected to MongoDB in getItems function.");
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
		 ...
		 //console.log("Successfully connected to MongoDB in getNumItems function.");
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        *for (var i=0; i<5; i++) {
		...
		array.push(new RegExp(query));
		
		var queryObject = {'$or' : [{'description': { '$in' : array }},{'slogan' : { '$in' : array }},{'title' : { '$in' : array }},{'category' : { '$in' : array }},{'img_url' : { '$in' : array }}]}
		
		this.db.collection('item').find(queryObject).sort({'_id' : 1}).skip( page * itemsPerPage ).limit(itemsPerPage).toArray(function(err, docs) {
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
		...
		this.db.collection('item').find(queryObject).toArray(function(err, docs) {
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
		...
		this.db.collection('item').findOneAndUpdate({'_id' : itemId},{'$push': {'reviews': reviewDoc}},{upsert: true, returnOriginal: false },function(err, result) {
                assert.equal(null, err);
